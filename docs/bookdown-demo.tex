...
\hypersetup{
  ...
  pdfauthor={Katherine E. Koziar \& Stephanie Labou},
  hidelinks,
...
\chapter{Workshop overview}\label{workshop-overview}

\subsection{BEGIN pitch()}\label{begin-pitch}
...
\subsection{STRUCTURE(workshop)}\label{structureworkshop}
...
\subsection{WHERE prior\_experience == FALSE}\label{where-prior_experience-false}
...
\subsection{FOR (x in example1:example5) \{annotate(x)\}}\label{for-x-in-example1example5-annotatex}
...
\subsection{IF attendee\_need == ``learn\_programming\_logic'': print(``register for this workshop!'')}\label{if-attendee_need-learn_programming_logic-printregister-for-this-workshop}
